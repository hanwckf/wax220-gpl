...
#define NMRP_LED_POWER_AMBER  "WAX220:amber:power"
#define NMRP_LED_POWER_GREEN  "WAX220:green:power"
#define NMRP_LED_POWER_BLUE  "WAX220:blue:power"
...
 
Upgrade Failed：
Power LED solid green and amner
Other off
*/
static void nmrp_ledInit(void)
{
    ...
    ret = led_get_by_label(NMRP_LED_POWER_BLUE, &dev);
    if (!ret)
    {
        led_set_state(dev, LEDST_ON);
    }

    ret = led_get_by_label(NMRP_LED_POWER_AMBER, &dev);
    if (!ret)
    {
        //led_set_period(dev, 1000);
...
static void nmrp_ledEnd(int result)
{
    ...
    if (result == 0)
    {
        ret = led_get_by_label(NMRP_LED_POWER_GREEN, &dev);
        if (!ret)
        {
            led_set_state(dev, LEDST_ON);
        }
    ...
    switch (NmrpState)
    {
        ...
        case NMRP_STATE_FILEUPLOADING:
        {
            if (NmrpFwUPOption)
            {
                ...
                ret = run_command("mtkupgrade fw now", 0);
    ...
    */
    ...
    if (ptOption != NULL)
    {
        ...
        */
        ...
        if ((wRegionNumber > 0) && (wRegionNumber < sizeof(Region) / sizeof(Region[0])))
        {
            int ret = 0;
