...
static int boot_from_ubi(struct mtd_info *mtd)
{
  ...
  if (!ret) {
    vol = find_ubi_volume("rootfs");
    if (vol)
      rootfs_size = vol->used_bytes;
  }

	if (ret || !rootfs_size || !strcmp(dual_image, "recover"))
  {
		cprintln(NORMAL, "*** Start recover image from backup ***");

    vol = find_ubi_volume("kernel_backup");
    if (!vol)
      goto out;

    kernel_size = vol->used_bytes;

    vol = find_ubi_volume("rootfs_backup");
    if (!vol)
      goto out;

    rootfs_size = vol->used_bytes;

    ret = read_ubi_volume("kernel_backup", (char *)data_load_addr, 0);
    if (ret)
      goto out;

    ret = read_ubi_volume("rootfs_backup", (char *)data_load_addr + kernel_size, 0);
    if (ret)
      goto out;

    ret = update_ubi_volume("kernel", -1, (char *)data_load_addr, kernel_size);
    if (ret)
      goto out;

    ret = update_ubi_volume("rootfs", -1, (char *)data_load_addr + kernel_size, rootfs_size);
    if (ret)
      goto out;

		cprintln(NORMAL, "*** Recover image from backup successfully ***");
  } else {
    if (!find_ubi_volume("kernel_backup") || !find_ubi_volume("rootfs_backup") || !strcmp(dual_image, "backup")) {
      cprintln(NORMAL, "*** Start update image to backup ***");

      vol = find_ubi_volume("kernel");
      if (!vol)
        goto out;

      kernel_size = vol->used_bytes;

      ret = read_ubi_volume("rootfs", (char *)data_load_addr + kernel_size, 0);
      if (!vol)
        goto out;

      ret = update_ubi_volume("kernel_backup", -1, (char *)data_load_addr, kernel_size);
      if (ret)
        goto out;

      ret = update_ubi_volume("rootfs_backup", -1, (char *)data_load_addr + kernel_size, rootfs_size);
      if (ret)
        goto out;

      cprintln(NORMAL, "*** Update image to backup successfully ***");
    }
  }

  if (strlen(dual_image) > 0) {
    if (strcmp(dual_image, "upgraded"))
      env_set("dual_image", NULL);
    else
      env_set("dual_image", "recover");
    env_save();
  }
