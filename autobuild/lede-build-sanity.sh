...
prepare() {
	...
	echo "[TW]  before del patch"
	rm  -f  feeds/mtk_openwrt_feed/openwrt_patches-21.02/107-strongswan-add-uci-support.patch
	rm  -f  autobuild/openwrt_patches-21.02/mtk_soc/0901-sbc-build-host-tool-cryptsetup.patch
	echo "[TW]  after del patch"

	#do mtk_feed prepare_sdk.sh
	cp ${MTK_FEED_DIR}/prepare_sdk.sh ${BUILD_DIR}

	echo "[TW]  after prepare_sdk.sh"
...
prepare_final() {
	...
	*)
	...
	echo "[TW]  do prepare fianl rm "
	rm -rf ./tmp
	#copy main test config(.config)
	echo "[TW]  do prepare fianl be copy_main_Config"
	copy_main_Config $1
	echo "[TW]  do prepare fianl af copy_main_Config"
	
	echo make defconfig
	make defconfig
	
	echo "[TW]  do prepare fianl af defconfig"
}
