...
void foe_clear_all_bind_entries(struct net_device *dev)
{
	...
	if (!IS_LAN(dev) && !IS_WAN(dev) &&
	    ...
	    !dev->netdev_ops->ndo_fill_forward_path)
		return;
				...
				struct sk_buff *skb, int val)
{
	if (IS_WHNAT(state->in) && FROM_WED(skb)) {
		...
		if (state->in->netdev_ops->ndo_fill_forward_path) {
					  ...
					  struct net_device_path *hw_path)
{
	...
	if (hw_path->type == DEV_PATH_PPPOE) {
		memcpy(eth_hdr(skb)->h_source, hw_path->encap.h_source, ETH_ALEN);
		memcpy(eth_hdr(skb)->h_dest, hw_path->encap.h_dest, ETH_ALEN);
		return 0;
	}
				  ...
				  struct net_device_path *hw_path)
{
	...
	case IPV4_HNAT:
		...
		entry.ipv4_hnapt.pppoe_id = (hw_path->type == DEV_PATH_PPPOE) ?
					     hw_path->encap.id : 0;
		break;
	case IPV4_DSLITE:
	case IPV4_MAP_E:
	...
	case IPV6_3T_ROUTE:
		...
		entry.ipv6_5t_route.pppoe_id = (hw_path->type == DEV_PATH_PPPOE) ?
						hw_path->encap.id : 0;
		break;
	}
	return entry;
...
struct foe_entry ppe_fill_info_blk(struct ethhdr *eth, struct foe_entry entry,
				   struct net_device_path *hw_path)
{
	entry.bfib1.psn = (hw_path->type == DEV_PATH_PPPOE) ? 1 : 0;
	entry.bfib1.vlan_layer += (hw_path->type == DEV_PATH_VLAN) ? 1 : 0;
				     ...
				     struct net_device_path *hw_path)
{
	...
	if (IS_LAN(dev)) {
		if (IS_DSA_LAN(dev))
			port_id = hnat_dsa_fill_stag(dev, &entry,
						     ctx, hw_path,
						     ntohs(eth->h_proto),
						     mape);
...
{
	struct foe_entry *entry;
	struct net_device_path_ctx ctx = { .dev	= out, };
	struct net_device_path hw_path = { .dev = out, };
	...
	if (out->netdev_ops->ndo_fill_forward_path) {
		out->netdev_ops->ndo_fill_forward_path(&ctx, &hw_path);
		out = (IS_GMAC1_MODE) ? hw_path.dev : ctx.dev;
	}
	...
	case HIT_UNBIND_RATE_REACH:
		...
		skb_to_hnat_info(skb, out, entry, &ctx, &hw_path);
		break;
