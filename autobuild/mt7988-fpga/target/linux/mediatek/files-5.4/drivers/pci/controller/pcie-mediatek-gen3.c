				    ...
				    unsigned long type, int num)
{
	...
	if (num >= PCIE_MAX_TRANS_TABLES) {
		dev_info(port->dev, "not enough translate table for addr: %#llx, limited to [%d]\n",
...
static int mtk_pcie_init_irq_domains(struct mtk_pcie_port *port)
{
	...
	if (!intc_node) {
		dev_info(dev, "missing interrupt-controller node\n");
		return -ENODEV;
	}
	...
	if (!port->intx_domain) {
		dev_info(dev, "failed to create INTx IRQ domain\n");
		return -ENODEV;
	}
...
static int mtk_pcie_parse_port(struct mtk_pcie_port *port)
{
	...
	if (IS_ERR(port->phy_reset)) {
		...
		if (ret != -EPROBE_DEFER)
			dev_info(dev, "failed to get PHY reset\n");
	...
	if (IS_ERR(port->mac_reset)) {
		...
		if (ret != -EPROBE_DEFER)
			dev_info(dev, "failed to get MAC reset\n");
	...
	if (IS_ERR(port->phy)) {
		...
		if (ret != -EPROBE_DEFER)
			dev_info(dev, "failed to get PHY\n");
...
static int mtk_pcie_power_up(struct mtk_pcie_port *port)
{
	...
	if (err) {
		dev_info(dev, "failed to initialize PHY\n");
		goto err_phy_init;
	}

	err = phy_power_on(port->phy);
	if (err) {
		dev_info(dev, "failed to power on PHY\n");
		goto err_phy_on;
	}
