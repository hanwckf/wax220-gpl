#ifndef __MTK_ETH_API_H__
#define __MTK_ETH_API_H__
...
#define MTK_ETH_API_BASE_CTL (64 + 1024 + 64 + 64 + 9000)

typedef enum mtk_eth_api_set_e
{
    MTK_ETH_API_BASE_SET_CTL = MTK_ETH_API_BASE_CTL,
    MTK_ETH_API_MDIO_C22_WRITE,
    MTK_ETH_API_MDIO_C45_WRITE,
    MTK_ETH_API_SWITCH_REG_WRITE,
    MTK_ETH_API_MDIO_SOC_C45_WRITE,
    MTK_ETH_API_PPE_TABLE_CLEAR,
    MTK_ETH_API_SET_CTL_MAX
} mtk_eth_api_set_t;

typedef enum mtk_eth_api_get_e
{
    MTK_ETH_API_BASE_GET_CTL = MTK_ETH_API_BASE_CTL,
    MTK_ETH_API_MDIO_C22_READ,
    MTK_ETH_API_MDIO_C45_READ,
    MTK_ETH_API_SWITCH_REG_READ,
    MTK_ETH_API_MDIO_SOC_C45_READ,
    MTK_ETH_API_GET_CTL_MAX
} mtk_eth_api_get_t;
...
typedef struct
{
    ...
    int addr;
    int data;
} mtk_eth_api_mdio_c45_t;

typedef struct
{
    int reg;
    int val;
} mtk_eth_api_switch_reg_t;

typedef union
{
    mtk_eth_api_mdio_c22_t c22;
    mtk_eth_api_mdio_c45_t c45;
    mtk_eth_api_switch_reg_t swreg;
} mtk_eth_api_union_t;
...
#define GETSOCKOPT(optid, varptr, vartype, qty)                                               \
    ...
    {                                                                                         \
        ...
        if ((sockfd = socket(AF_INET, SOCK_RAW, IPPROTO_RAW)) == -1)                          \
        {                                                                                     \
            printf("%s %d get socket fail\r\n", __FUNCTION__, __LINE__);                      \
            perror("socket");                                                                 \
            return -1;                                                                        \
        }                                                                                     \
        len = sizeof(vartype) * qty;                                                          \
        if ((ret = getsockopt(sockfd, IPPROTO_IP, optid, (void *)varptr, (void *)&len)) != 0) \
        {                                                                                     \
            printf("%s %d getsockopt fail ret = 0x%x\r\n", __FUNCTION__, __LINE__, ret);                      \
            perror("getsockopt");                                                             \
            close(sockfd);                                                                    \
            return ret;                                                                       \
        }                                                                                     \
