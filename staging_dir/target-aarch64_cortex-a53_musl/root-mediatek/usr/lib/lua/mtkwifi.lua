...
function mtkwifi.__config_mac_filter(ifname, filter, allow, deny)
    local allowmacs
    local denymacs

    allowmacs = string.gsub(allow, " ", ";")
    denymacs = string.gsub(deny, " ", ";")

    os.execute("iwpriv "..ifname.." set MacFilterClearAll=1 > /dev/null 2>&1")

    if filter == 'allow' then
        os.execute("iwpriv "..ifname.." set MacFilterPolicy=1 > /dev/null 2>&1")
        os.execute("iwpriv "..ifname.." set MacFilterAddEntry=\""..allowmacs.."\" > /dev/null 2>&1")
    elseif filter == 'deny' then
        os.execute("iwpriv "..ifname.." set MacFilterPolicy=2 > /dev/null 2>&1")
        os.execute("iwpriv "..ifname.." set MacFilterAddEntry=\""..denymacs.."\" > /dev/null 2>&1")
    else
        os.execute("iwpriv "..ifname.." set MacFilterPolicy=0 > /dev/null 2>&1")
    end

    os.execute("iwpriv "..ifname.." set MacFilterShowAll=1 > /dev/null 2>&1")
end
