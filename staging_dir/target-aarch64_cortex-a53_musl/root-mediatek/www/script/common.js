...
function showStaHWMode(m)
{
    ...
    var mode = {"802.11ng":"n/g/b", "802.11g":"g/b", "802.11b":"b", "only":"g", "802.11n":"n", "802.11a":"a", "802.11ac":"ac/n/a", "802.11na":"n/a", "802.11axg":"ax", "802.11axa":"ax", "802.11":""};
    return "802.11 " + mode[m];
}
function showOpmode(s)
{
    var mode = {"sta":"Client Bridge", "wds_sta":"AirBridge Satellite", "ap":"Access Point", "wds_ap":"AirBridge Master", "wds_bridge":"WDS Bridge", "sta_ap":"Repeater"};
    return mode[s];
}
function showBandwidth(b, disp)
{
    disp = disp || "";
    var band = {"HT20_40":((disp == "wireless_setting")?"20/40":"20-40"), "HT20":"20", "HT40":"40", "HT80":"80", "HT160":"160"};
    return band[b] + " MHz";
}
function showFrequency(m)
{
    var mode = {"1":"2.412", "2":"2.417", "3":"2.422", "4":"2.427", "5":"2.432", "6":"2.437", "7":"2.442", "8":"2.447", "9":"2.452", "10":"2.457", "11":"2.462", "12":"2.467", "13":"2.472",
        "36":"5.180", "40":"5.200", "44":"5.220", "48":"5.240", "52":"5.260", "56":"5.280", "60":"5.300", "64":"5.320",
        "100":"5.500","104":"5.520","108":"5.540", "112":"5.560", "116":"5.580", "120":"5.600", "124":"5.620", "128":"5.640", "132":"5.660", "136":"5.680", "140":"5.700", "144":"5.720",
        "149":"5.745", "153":"5.765", "157":"5.785", "161":"5.805", "165":"5.825",
        "169":"5.845", "173":"5.865", "177":"5.885", "181":"5.905"};
    return mode[m] + " GHz";
}
function showCountry(m)
{
    var mode = {"356": "$India", "392": "$Japan", "233": "$Estonia", "276": "$Germany", "372": "$Ireland", "616": "$Poland", "250": "$France", "578": "$Norway", "752": "$Sweden", "410": "$South_Korea", "756": "$Switzerland", "246": "$Finland", "40": "$Austria", "36": "$Australia", "56": "$Belgium", "442": "$Luxembourg", "703": "$Slovakia", "554": "$New_Zealand", "208": "$Denmark", "380": "$Italy", "826": "$United_Kingdom", "724": "$Spain", "705": "$Slovenia", "440": "$Lithuania", "352": "$Iceland", "203": "$Czech_Republic", "191": "$Croatia", "642": "$Romania", "348": "$Hungary", "300": "$Greece", "620": "$Portugal", "100": "$Bulgaria", "528": "$Netherlands", "428": "$Latvia", "840": "$United_States"};    
    return mode[m];
}
...
function ajax_normal(params) {
  function check_response(cb, data) {
    var _data = '';
    ...
    // console.log("ajax_post response data:", _data);
    if (_data && "string" == typeof(_data) &&
        (_data.indexOf("<html>") != -1 || _data.indexOf("<HTML>") != -1)) {
      if (_data.indexOf("redirect_flag") != -1) {
        var _v = _data;
        var _i = _v.indexOf("page");
        if (_i != -1) {
          _v = _v.slice(_i);
          _i = _v.indexOf("\"");
          if (_i != -1) {
            ...
            _i = _v.indexOf("\"");
            if (_i != -1) {
              var _path = _v.substring(0, _i);
              console.log("redirect to", _path);
              getTop(window).location.href = "/" + _path;
              return;
            }
          }
        }
  ...
  if (typeof(params) != 'object') { console.log("ajax_normal: invalid params type."); return; }
  var _params = {};
  for (var _n in params) {
    if (typeof(_n) != 'string') { continue; }

    _params[_n] = params[_n];

    if (_n.toLowerCase() == 'success') {
      _params[_n] = function(output) { check_response(params[_n], output)};
      continue;
    }

    if (_n.toLowerCase() == 'error') {
      _params[_n] = function(err) { check_response(params[_n], err)};
      continue;
    }

    if (_n.toLowerCase() == 'url' && typeof(params[_n]) == 'string') {
      var _a = params[_n].split(".");
      var _s = _a[_a.length-1].toLowerCase();
      if (_s == "htm" || _s == "html") {
        continue;
      }

      var _k = 'timestamp=';
      if (-1 == params[_n].indexOf(_k)) {
        _params[_n] = params[_n] + ' ' + _k + get_timestamp();
      } else {
        var _b = params[_n].lastIndexOf(_k) + _k.length;
        var _e = parseInt(params[_n].slice(_b)).toString().length + _b;
        _params[_n] = params[_n].substring(0, _b) + get_timestamp() + params[_n].slice(_e);
      }
    }
  }
  // console.log("ajax_normal input:", params);
  // console.log("ajax_normal   new:", _params);
  $$.ajax(_params);
}
