...
function __set_wifi_security(cfgs, diff, device, devname)
    ...
    for i, vif in ipairs(vifs) do
        ...
        elseif vif.AuthMode == "WPA1WPA2" then
            ...
            vif.vifname, vif.vifname, vif.EncrypType, vif.vifname, vif.RADIUS_Server,
            vif.vifname,vif.RADIUS_Port, vif.vifname, vif.RADIUS_Key, vif.vifname)
        elseif vif.AuthMode == "WPA2WPA3" then
            commands = string.format([[
                iwpriv %s set AuthMode=WPA2WPA3;
                iwpriv %s set EncrypType=%s;
                iwpriv %s set RADIUS_Server=%s;
                iwpriv %s set RADIUS_Port=%s;
                iwpriv %s set RADIUS_Key="%s";
...
function __set_wifi_misc(cfgs, diff, device,devname)
    ...
    for k,v in pairs(diff) do
        ...
        elseif k == "Channel" or k == "channel" then
            ...
            os.execute("echo [quick_settings] Channel bssid_str = "..bssid_str.." >> /tmp/wifiLuaDebug.log")
            for i=1, bssid_str  do
                if v[2] == "0" then
                    commands_vifs = string.format([[
                    iwpriv %s set AutoChannelSel=3;]], vifext..tostring(tonumber(i)-1))
                    debug_info_write(devname, commands_vifs)
                    os.execute("echo [quick_settings] Channel AutoChannelSel = "..devname.." ifname = "..vifext..tostring(tonumber(i)-1).." >> /tmp/wifiLuaDebug.log")
                else
                    commands_vifs = string.format([[
                    iwpriv %s set Channel=%s;]], vifext..tostring(tonumber(i)-1), tostring(v[2]))
                    debug_info_write(devname, commands_vifs)
                    os.execute("echo [quick_settings] Channel devname = "..devname.." ifname = "..vifext..tostring(tonumber(i)-1).." >> /tmp/wifiLuaDebug.log")
                end
            end
        ...
        elseif k == "PowerUpVHT160" or k == "powerupvht160" then
            ...
            commands = string.format([[
                iwpriv %s set TxPowerBoostCtrl=%s;]], vifname, tostring(val))
        elseif k == "RadioMaxStaNum" or k == "radiomaxstanum" then
            commands = string.format([[
                iwpriv %s set RadioMaxStaNum=%s;]], vifname, tostring(v[2]))
        ...
        elseif k == "suprateset" then
            local bssid_str = tonumber(bssid_num)
            os.execute("echo [quick_settings] suprateset  bssid_str = "..bssid_str.." >> /tmp/wifiLuaDebug.log")
            for i=1, bssid_str  do
               commands_vifs = string.format([[
               iwpriv %s set suprateset=%s;]], vifext..tostring(tonumber(i)-1), tostring(v[2]))
               debug_info_write(devname, commands_vifs)
               os.execute("echo [quick_settings] suprateset devname = "..devname.." ifname = "..vifext..tostring(tonumber(i)-1).." >> /tmp/wifiLuaDebug.log")
            end
        ...
        elseif k:find("L2WhiteMacList") then
            local index = string.match(k, '%d', 3)
			local commands_maclist = ""
			local macList = ""
			os.execute("echo [quick_settings] index = "..vifext..tostring(index).." >> /tmp/wifiLuaDebug.log")
            -- Clear all entry first
			if diff["L2WhiteMacList"..tostring(index)] and diff["L2WhiteMacList"..tostring(index)][2] ~= "" then
				-- os.execute("echo [quick_settings] commands_maclist a2= "..commands_maclist.." >> /tmp/wifiLuaDebug.log")
				macList = "\""..diff["L2WhiteMacList"..tostring(index)][2].."\""
				-- os.execute("echo [quick_settings] macList = "..macList.." >> /tmp/wifiLuaDebug.log")
				
                commands_maclist = string.format([[
                    iwpriv %s set L2WhiteMacListDelAll=1;]], vifext..tostring(index))
					os.execute("echo [quick_settings] commands_maclist 4= "..commands_maclist.." >> /tmp/wifiLuaDebug.log")
                debug_info_write(devname, commands_maclist)
				
                commands_maclist = string.format([[
                    iwpriv %s set L2WhiteMacListSet=%s;]], vifext..tostring(index), macList)
				-- os.execute("echo [quick_settings] commands_maclist 2.1 = "..commands_maclist.." >> /tmp/wifiLuaDebug.log")		
                debug_info_write(devname, commands_maclist)
			else
				-- os.execute("echo [quick_settings] commands_maclist 3= "..commands_maclist.." >> /tmp/wifiLuaDebug.log")
                commands_maclist = string.format([[
                    iwpriv %s set L2WhiteMacListDelAll=1;]], vifext..tostring(index))
					os.execute("echo [quick_settings] commands_maclist 4= "..commands_maclist.." >> /tmp/wifiLuaDebug.log")
                debug_info_write(devname, commands_maclist)
				-- os.execute("echo [quick_settings] commands_maclist 5= "..commands_maclist.." >> /tmp/wifiLuaDebug.log")
			end
 
			-- os.execute("echo [quick_settings] commands_maclist = "..commands_maclist.." >> /tmp/wifiLuaDebug.log")
            commands_access_2[index] = true
...
function quick_settings(devname,path)
    ...
    need_downup_parms = {"HT_LDPC", "VHT_SGI", "VHT_LDPC", "idle_timeout_interval",
                          ...
                          "TxPower", "radio", "RadioOn", "CountryRegionABand", "AutoChannelSkipList", "FtSupport"}
