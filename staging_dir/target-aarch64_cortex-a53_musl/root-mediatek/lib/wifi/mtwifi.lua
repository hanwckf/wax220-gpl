...
function d8021xd_setup(devname)
    ...
    if l1parser and devs then
        ...
        if not dev then
            nixio.syslog("err", "d8021xd_setup: dev "..devname.." not found!")
            return
        end
...
function mtwifi_reload(devname)
    ...
    if not devname then
        ...
        if not mtkwifi.exists("/tmp/mtk/wifi/normal_reload") then
            os.execute("echo [mtwifi_reload] not exists /tmp/mtk/wifi/normal_reload file >> /tmp/wifiLuaDebug.log")
            _G["doBrctlAddIf"](0)
        else
        ...
        for devname, dev in mtkwifi.__spairs(devs.devname_ridx) do
            if qsetting then
                ...
                os.execute("echo [mtwifi_reload] path = "..path.." >> /tmp/wifiLuaDebug.log")
                if not mtkwifi.exists("/tmp/mtk/wifi/"..string.match(path, "([^/]+)\.dat")..".applied") then
                    ...
                    os.execute("echo [mtwifi_reload] not exist xxx.applied file, and creat xxx.last file >> /tmp/wifiLuaDebug.log") 
                else
                    ...
                    os.execute("echo [mtwifi_reload] devname = "..devname.." path = "..path.." >> /tmp/wifiLuaDebug.log")
                    if normal_reload then
                        os.execute("echo [mtwifi_reload] normal_reload is true >> /tmp/wifiLuaDebug.log")
                        break
                    end
                end
            end
...
function mtwifi_reload8021x(devname)
    local nixio = require("nixio")
    local mtkwifi = require("mtkwifi")
    local profiles
    local devs, l1parser = mtkwifi.__get_l1dat()
